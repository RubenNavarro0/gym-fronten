    ...
    stats.value = [
      {
        ...
        value: data.totalClientes.toLocaleString('es'),
        trend: '', up: true,
        bg: 'rgba(255,92,0,0.15)', icon: '👥'
      },
      {
        label: 'Ingresos del mes',
        value: '€' + Number(data.ingresosMes).toLocaleString('es'),
        trend: '', up: true,
        bg: 'rgba(0,230,118,0.12)', icon: '💰'
      },
      {
        label: 'Vencen hoy',
        value: String(data.vencenHoy),
        trend: '', up: false,
        bg: 'rgba(255,60,60,0.12)', icon: '⏰'
      },
      {
        label: 'Altas este mes',
        value: String(data.altasMes),
        trend: '', up: true,
        bg: 'rgba(100,100,255,0.12)', icon: '⚡'
      },
    ]
  } catch (e) {
...
async function fetchMembers() {
  try {
    ...
    console.error('Error miembros:', e)
  } finally {
...
.member-name {
  font-size: 13px; font-weight: 500; color: #d8d8e8;
...
.capacity-bar {
  flex: 1; height: 6px;
  background: #1e1e2a; border-radius: 10px; overflow: hidden;
}
.capacity-fill {
  height: 100%;
  background: linear-gradient(90deg,#ff5c00,#ffa000);
