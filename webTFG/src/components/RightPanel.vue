  ...
  <div class="right-panel">
      ...
      <div class="action-buttons">
        <button class="action-btn primary" @click="openModal('add')">
          ...
          <div class="action-text">
            <span class="action-label">Añadir Cliente</span>
            <span class="action-sub">Registrar nuevo socio</span>
          </div>
        ...
        <button class="action-btn" @click="openModal('renew')">
          <span class="action-icon">🔄</span>
          <div class="action-text">
            <span class="action-label">Renovar Suscripción</span>
            <span class="action-sub">Actualizar plan existente</span>
          </div>
          <span class="action-arrow">→</span>
        </button>
        <button class="action-btn" @click="openModal('edit')">
          ...
          <div class="action-text">
            <span class="action-label">Modificar Cliente</span>
            <span class="action-sub">Editar datos del socio</span>
          </div>
          <span class="action-arrow">→</span>
        </button>
        <button class="action-btn danger" @click="openModal('checkin')">
          <span class="action-icon">✅</span>
          <div class="action-text">
            <span class="action-label">Check-in Manual</span>
            <span class="action-sub">Registrar ingreso</span>
          </div>
          <span class="action-arrow">→</span>
        </button>
      </div>
    ...
    <div class="section">
      ...
      <div class="alerts-list">
        ...
        <p v-if="!alertas.vencidas && !alertas.vencenHoy && !alertas.altasMes" class="empty-msg">
          Sin alertas activas
        </p>
      </div>
    </div>
        ...
        <div class="modal-body">
          ...
          <template v-if="modal === 'renew'">
            <div class="form-group">
              ...
              <SearchCliente :id_gym="gymId" @seleccionado="seleccionarCliente" />
              <p class="selected-label" v-if="form.id_cliente">
                ✓ <strong>{{ form.nombreCliente }}</strong>
              </p>
            </div>
            <div class="form-group">
...
async function fetchAlertas() {
  try {
    ...
    alertas.value = {
      vencenHoy: stats.data.vencenHoy,
      altasMes:  stats.data.altasMes,
      vencidas:  resumen.data.vencidas
    }
  } catch { /* silencioso */ }
}
...
async function submitModal() {
  ...
  try {
    if (modal.value === 'add') {
      ...
      feedback.value.ok = 'Cliente registrado correctamente'
    }
    ...
    if (modal.value === 'renew') {
      ...
      await api.post(`/cliente/${form.value.id_cliente}/renovar`, {
        id_plan: Number(form.value.id_plan)
      })
      feedback.value.ok = 'Suscripción renovada correctamente'
    }
    ...
    if (modal.value === 'edit') {
      ...
      feedback.value.ok = 'Cliente actualizado correctamente'
    }

    if (modal.value === 'checkin') {
      if (!form.value.id_cliente) throw new Error('Selecciona un socio primero')
      const { data } = await api.post('/registroentrada/checkin', {
        busqueda: form.value.nombreCliente
      })
      feedback.value.ok = `${data.message} (accesos restantes: ${data.accesos_restantes})`
    }
...
.section-title {
  ...
  font-size: 13px; letter-spacing: 2.5px; color: #44445a; margin: 0 0 14px;
}
...
.plan-card  { background: #16161c; border: 1px solid #1e1e2a; border-radius: 12px; padding: 14px; transition: border-color 0.2s; }
