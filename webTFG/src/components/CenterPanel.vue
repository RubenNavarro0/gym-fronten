  ...
  <div class="center-panel">
    ...
    <div class="panel-header">
      ...
      <div class="header-actions">
        <input class="search-input" type="text" placeholder="Buscar socio..." v-model="searchQ" @input="onSearch" />
        <button class="btn-add" @click="$emit('openModal', 'add')">+ Nuevo</button>
      </div>
    </div>
    ...
    <div v-if="activeTab === 'cuotas'" class="tab-content">
      ...
      <div class="cuotas-summary" v-else>
        <div class="summary-card highlight">
          ...
          <div class="mini-bar">
            <div class="mini-fill" :style="{ width: resumen.facturado > 0 ? '100%' : '0%' }"></div>
          </div>
        </div>
      ...
      <div class="chart-section">
        ...
        <div v-if="loadingBars" class="bar-chart">
          <div class="bar-group" v-for="i in 6" :key="i">
            <div class="bar-wrap"><div class="bar-fill skeleton-bar"></div></div>
            ...
            <div class="bar-wrap">
              <div
                ...
                :style="{
                  height: b.pct + '%',
                  background: b.active
                    ? 'linear-gradient(180deg,#ff8c00,#ff5c00)'
                    : '#1e1e2a'
                }"
              ></div>
            </div>
      ...
      <div class="table-section">
        ...
        <table class="data-table" v-else>
          <thead>
            <tr>
              <th>Socio</th><th>Plan</th><th>Monto</th><th>Vencimiento</th><th>Estado</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="q in cuotas" :key="q.id">
              <td>
                <div class="td-user">
                  <div class="td-avatar" :style="{ background: avatarColor(q.nombre) }">
                    {{ initials(q.nombre) }}
                  </div>
                  {{ q.nombre }}
                </div>
              ...
              <td>
                <span class="status-pill" :class="estadoClass(q.estado)">
    ...
    <!-- TAB: Info General (estática, información del gym) -->
...
function estadoClass(e) {
  return { activa: 'paid', inactiva: 'pending', vencida: 'overdue' }[e] ?? 'pending'
}
...
async function fetchResumen() {
  try {
    const { data } = await api.get('/suscripcion/resumen')
    resumen.value = data
...
async function fetchBars() {
  try {
    const { data } = await api.get('/suscripcion/ingresos-mensuales')
    barData.value = data
  } catch (e) { console.error(e) }
  finally { loadingBars.value = false }
}

async function fetchCuotas() {
  try {
    const { data } = await api.get('/suscripcion/recientes')
    cuotas.value = data
  } catch (e) { console.error(e) }
...
.summary-card.highlight {
  ...
  background: linear-gradient(135deg, rgba(255,92,0,0.08), rgba(255,140,0,0.04));
}
.summary-num {
  font-family: 'Bebas Neue', sans-serif;
...
.summary-label { font-size: 11px; color: #44445a; font-weight: 400; }
...
.bar-wrap {
  flex: 1; width: 100%;
  display: flex; align-items: flex-end;
  background: #16161c; border-radius: 6px; overflow: hidden;
}
...
.data-table thead tr th {
  ...
  letter-spacing: 1.5px; color: #33334a;
  text-transform: uppercase; border-bottom: 1px solid #1e1e24;
}
.data-table tbody tr {
...
.td-amount { color: #e8e8ee; font-weight: 600; }
.td-date   { color: #55556a; font-size: 12px; }
...
.class-card {
  ...
  padding: 14px 16px; background: #16161c;
